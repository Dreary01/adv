...
func (h *ObjectTypeHandler) List(w http.ResponseWriter, r *http.Request) {
	rows, err := h.db.Query(context.Background(),
		`SELECT id, name, description, kind, icon, color, can_be_root,
		 ...
		 FROM object_types ORDER BY sort_order, name`)
	if err != nil {
...
func (h *ObjectTypeHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	childRows, _ := h.db.Query(context.Background(),
		`SELECT child_type_id FROM object_type_hierarchy WHERE parent_type_id = $1`, id)
	if childRows != nil {
	...
	parentRows, _ := h.db.Query(context.Background(),
		`SELECT parent_type_id FROM object_type_hierarchy WHERE child_type_id = $1`, id)
	if parentRows != nil {
	...
	refRows, _ := h.db.Query(context.Background(),
		`SELECT ref_table_id FROM object_type_ref_tables WHERE object_type_id = $1 ORDER BY sort_order`, id)
	if refRows != nil {
		defer refRows.Close()
		for refRows.Next() {
			var refID string
...
func (h *ObjectTypeHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	var t models.ObjectType
	err := h.db.QueryRow(context.Background(),
		`INSERT INTO object_types (name, description, kind, icon, color, can_be_root,
		                           default_duration_days, auto_fill_effort, add_to_calendar,
...
func (h *ObjectTypeHandler) Delete(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "id")
	_, err := h.db.Exec(context.Background(), `DELETE FROM object_types WHERE id = $1`, id)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *ObjectTypeHandler) SetHierarchy(w http.ResponseWriter, r *http.Request) {
	...
	tx.Exec(context.Background(), `DELETE FROM object_type_hierarchy WHERE parent_type_id = $1`, id)
	for _, childID := range req.ChildTypeIDs {
		tx.Exec(context.Background(),
			`INSERT INTO object_type_hierarchy (parent_type_id, child_type_id) VALUES ($1, $2)
			 ON CONFLICT DO NOTHING`, id, childID)
	}
...
func (h *ObjectTypeHandler) BindRequisite(w http.ResponseWriter, r *http.Request) {
	...
	err := h.db.QueryRow(context.Background(),
		`INSERT INTO object_type_requisites
		 (object_type_id, requisite_id, is_required, is_visible, is_lockable,
		...
		typeID, req.RequisiteID, req.IsRequired, req.IsVisible, req.IsLockable,
...
func (h *ObjectTypeHandler) UnbindRequisite(w http.ResponseWriter, r *http.Request) {
	...
	h.db.Exec(context.Background(),
		`DELETE FROM object_type_requisites WHERE object_type_id = $1 AND requisite_id = $2`,
		typeID, reqID)
	w.WriteHeader(http.StatusNoContent)
}
...
func (h *ObjectTypeHandler) ListRefTables(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Query(context.Background(),
		`SELECT rt.id, rt.name, rt.description, rt.icon, rt.structure, rt.input_mode,
		 ...
		 WHERE otr.object_type_id = $1
		 ORDER BY otr.sort_order`, typeID)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *ObjectTypeHandler) BindRefTable(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.db.Exec(context.Background(),
		`INSERT INTO object_type_ref_tables (object_type_id, ref_table_id, sort_order)
		 VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`,
		typeID, req.RefTableID, req.SortOrder)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *ObjectTypeHandler) UnbindRefTable(w http.ResponseWriter, r *http.Request) {
	...
	h.db.Exec(context.Background(),
		`DELETE FROM object_type_ref_tables WHERE object_type_id = $1 AND ref_table_id = $2`,
		typeID, tableID)
	w.WriteHeader(http.StatusNoContent)
}
